...
export function FontSelector({ fonts, selectedFontId, onSelect, label }: FontSelectorProps) {
  ...
  if (fonts.length === 0) {
    return <div className='text-sm text-muted-foreground italic'>No hay recursos disponibles para esta área.</div>;
  }
        ...
        {fonts.map((font) => (
          ...
          >
            ...
            <div className='flex-1 min-w-0'>
              ...
              {font.description && <p className='text-xs text-muted-foreground line-clamp-1'>{font.description}</p>}
            </div>
            {selectedFontId === font.id && (
